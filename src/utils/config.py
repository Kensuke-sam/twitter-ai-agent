...
def load_config(project_root: Path | None = None) -> AppConfig:
    ...
    config_path = Path(config_override).expanduser().resolve() if config_override else root / "config.json"
    ...
    twitter_cli_path = Path(twitter_cli_path_raw).expanduser().resolve() if twitter_cli_path_raw else None
    ...
    return AppConfig(
        ...
        articles_file=_resolve_path(config_base, str(data.get("articles_file", "data/articles.json")), "data/articles.json"),
        drafts_file=_resolve_path(config_base, str(data.get("drafts_file", "data/drafts.json")), "data/drafts.json"),
        history_file=_resolve_path(config_base, str(data.get("history_file", "data/history.json")), "data/history.json"),
        max_tweet_length=int(data.get("max_tweet_length", 140)),
        default_candidate_count=int(data.get("default_candidate_count", 5)),
